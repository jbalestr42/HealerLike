...
namespace HealerLike.Render.Environment
{
    ...
    public class HLEnvironmentScatter : MonoBehaviour
    {
        ...
        [SerializeField] HLEnvironmentGust _gust;
        ...
        public HLEnvironmentSettings settings { get { return _settings; } set { _settings = value; } }

        List<HLEnvironmentItem> _items = new List<HLEnvironmentItem>();
        public IReadOnlyList<HLEnvironmentItem> items { get { return _items; } }
        ...
        public void ConfigureMotion(Camera viewCamera, HLEnvironmentGust gust, float fogEnd)
        {
        ...
        public void Build(Rect gridRect, float cellSize)
        {
            ...
            foreach (HLEnvironmentItem item in _items)
            {
                Spawn(item);
            }
        }
        ...
        void Spawn(HLEnvironmentItem item)
        {
            ...
            HLStoneRandom random = new HLStoneRandom(item.seed);
            float s = item.scale;
            _colourSeed = item.seed;
            switch (item.kind)
            {
                case HLEnvironmentKind.Boulder:
                    Stone(pivot, item.seed, HLStonePresets.Boulder, Vector3.zero, s, item.paletteIndex);
                    break;
                case HLEnvironmentKind.Monolith:
                    int palette = item.paletteIndex % 2 == 0 ? 1 : 2;
                    Stone(pivot, item.seed, HLStonePresets.Monolith, Vector3.zero, s, palette);
                    break;
        ...
        void SpawnCairn(Transform pivot, HLEnvironmentItem item, HLStoneRandom random, float s)
        {
            ...
            for (int i = 0; i < layers; i++)
            {
                ...
                uint seed = HLStoneSeed.ForPart(item.seed, (uint)i + 1);
                Vector3 part = Stone(pivot, seed, HLStonePresets.Cairn, bottom, k, (item.paletteIndex + i) & 3);
                y += part.y * 0.8f;
            }
        }

        void SpawnMushroomTree(Transform pivot, HLEnvironmentItem item, HLStoneRandom random, float s)
        {
        ...
        void SpawnSpiralFern(Transform pivot, HLEnvironmentItem item, HLStoneRandom random, float s)
        {
        ...
        void SpawnBladeRosette(Transform pivot, HLEnvironmentItem item, HLStoneRandom random, float s)
        {
        ...
        void SpawnSphereCluster(Transform pivot, HLEnvironmentItem item, HLStoneRandom random, float s)
        {
        ...
        {
            Mesh mesh = HLStoneMesh.CreateMesh(seed, shape);
            ...
            Color color = HLStoneAssembly.Palette[Mathf.Clamp(palette, 0, 3)];
