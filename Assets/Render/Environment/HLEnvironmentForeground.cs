...
namespace HealerLike.Render.Environment
{
    ...
    public class HLEnvironmentForeground : MonoBehaviour
    {
            ...
            for (int side = 0; side < 2; side++)
            {
                ...
                for (int n = 0; n < boulders + rosettes; n++)
                {
                    ...
                    for (int attempt = 0; attempt < 16; attempt++)
                    {
                        ...
                        bool isClear = true;
                        foreach (HLForegroundItem other in result)
                        {
                            float minDistance = 0.8f * (other.radius + radius);
                            if (other.kind == HLForegroundKind.Boulder && isBoulder
                    ...
                    {
                        kind = isBoulder ? HLForegroundKind.Boulder : HLForegroundKind.Rosette,
                        position = position,
                        scale = scale,
                        radius = radius,
                        yaw = random.Range(0f, 360f),
                        seed = HLStoneSeed.ForPart(baseSeed, (uint)(side * 256 + n + 1))
                    });
                }
            }
        ...
        public void Build(Vector3 cameraPosition, Quaternion cameraRotation, float verticalFov, float frameAspect)
        {
            ...
            foreach (HLForegroundItem item in _items)
            {
                Spawn(item);
            }
        }
        ...
        void Spawn(HLForegroundItem item)
        {
            ...
            {
                Mesh mesh = HLStoneMesh.CreateMesh(item.seed, HLStonePresets.Boulder);
