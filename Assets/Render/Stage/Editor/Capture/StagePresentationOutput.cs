...
namespace HealerLike.Render.Stage
{
    ...
    public sealed class StagePresentationOutput
    {
        [Serializable]
        public sealed class Manifest
        {
            ...
            public string source = "RenderStage actual Game view; production LateUpdate; held samples through StandaloneInputModule and StageTouchInput.Update";
            public string blindSpot = "Editor Metal capture with synthetic OS touch samples; no physical Android input, appearance or device performance evidence";
        ...
        [Serializable]
        public sealed class GrowthFrame
        {
        ...
        public void Check(bool result, string message) { ui.Check(result, message); }
        ...
        public void ExportPortrait(Texture2D source, string name)
        {
            ...
            try
            {
                ...
                {
                    if (Mathf.Abs(pixel.r - corner.r) + Mathf.Abs(pixel.g - corner.g) + Mathf.Abs(pixel.b - corner.b) > 24)
                        foreground++;
                }
                Check(foreground > pixels.Length / 100, "Actual cached portrait contains visible rendered detail: " + name);
        ...
        public GrowthFrame Sample(CreatureRig rig, Camera camera, string subject, string sourceAsset,
            float started, string screenshot)
        {
            GrowthFrame frame = new GrowthFrame {
                subject = subject, sourceAsset = sourceAsset, file = screenshot,
                gameFrame = Time.frameCount, width = Screen.width, height = Screen.height,
                realTime = Time.realtimeSinceStartupAsDouble, elapsed = Time.unscaledTime - started,
                appearanceElapsed = rig.appearanceElapsed, isAppearing = rig.isAppearing,
        ...
        public IEnumerator VerifyImages()
        {
            foreach (GrowthFrame frame in manifest.growth)
            {
                ...
                {
                    if (Time.realtimeSinceStartup >= deadline) Check(false, "Timed growth screenshot missing: " + frame.file);
                    yield return null;
                }
                Texture2D image = new Texture2D(2, 2, TextureFormat.RGB24, false);
                bool decoded = image.LoadImage(File.ReadAllBytes(path));
                bool matches = decoded && image.width == frame.width && image.height == frame.height;
        ...
        {
            if (!manifest.failures.Contains(message)) manifest.failures.Add(message);
            Write(false);
        }
    }
}
