    ...
    public static class HLEventCapture
    {
        ...
        static void Changed(PlayModeStateChange state)
        {
            ...
            if(state==PlayModeStateChange.EnteredPlayMode)
            {
                ...
                events.Clear(); stillFrames.Clear(); pendingStills.Clear(); seen.Clear(); allies.Clear(); listeners.Clear(); buffManagers.Clear(); hostileFinishAt=0; statuses=hostileAreas=0; seenAreas.Clear(); speedValid=true; overviewRequested=overviewVerified=refocusRequested=false;
        ...
        static void Late()
        {
            ...
            if(Hostile) {
                if(hostileFinishAt>0 && t>=hostileFinishAt) Finish(hostileAreas>0 && negativeHealth>0 && speedValid,"supplemental actual debug-inventory hostile area plus damage; source and radius logged; waited for queued still");
