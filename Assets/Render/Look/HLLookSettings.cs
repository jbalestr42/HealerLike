...
{
    /// <summary>Authoring colours are sRGB. Validation returns a copy.</summary>
    [Serializable]
    public struct HLLookSettings
    {
        ...
        public HLLookSettings Validated()
        {
            ...
            value.ShadowStrength = Mathf.Clamp(Finite(ShadowStrength, defaults.ShadowStrength), .01f, 1f);
            value.ToonThreshold = Mathf.Clamp(Finite(ToonThreshold, defaults.ToonThreshold), .001f, .999f);
            ...
            value.InkRange = Mathf.Max(.001f, Finite(InkRange, defaults.InkRange));
            value.DensityMul = Mathf.Clamp(Finite(DensityMul, defaults.DensityMul), .01f, 1f);
            ...
            value.DashAmount = Mathf.Clamp(Finite(DashAmount, defaults.DashAmount), 0f, .92f);
            value.DashScale = Mathf.Max(.001f, Finite(DashScale, defaults.DashScale));
            value.InkDistStart = Mathf.Max(.001f, Finite(InkDistStart, defaults.InkDistStart));
            value.InkFarSpacing = Mathf.Max(0f, Finite(InkFarSpacing, defaults.InkFarSpacing));
            value.FogBands = Mathf.Max(1, FogBands);
            // At the largest float there is no finite end greater than start.
            ...
            if ((double)value.FogEnd - value.FogStart < .001)
            {
                ...
                // Round upward if .001 is smaller than the float spacing, or rounded down.
                if ((double)value.FogEnd - value.FogStart < .001)
        ...
        private static Color ValidateColor(Color value, Color fallback, bool nonblack)
        {
