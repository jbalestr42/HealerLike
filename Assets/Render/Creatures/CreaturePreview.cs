...
namespace HealerLike.Render.Creatures
{
    ...
    public sealed class CreaturePreview : IDisposable
    {
        ...
        public bool Init(CreatureLooks looks, EntityData data, Entity.EntityType side, PrimitiveMeshes meshes,
            Transform parent, float cellSize)
        {
            if (_disposed || _rig != null || !looks || !data || !parent) return false;
            GameObject view = looks.GetView(data, side);
            CreatureBuilder host = view ? view.GetComponentInChildren<CreatureBuilder>(true) : null;
            if (!host || !host.material) return false;
            ...
            if (!created.Init(recipe, parent, host.material, host.bodyMaterial,
                host.meshes ? host.meshes : meshes, cellSize))
            {
                created.Dispose();
        ...
        {
            if (_rig != null) _rig.CompleteAppearance();
        }
