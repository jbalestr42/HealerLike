...
{
    public class StoneEnemyVisual : MonoBehaviour, IEntityView, IDeliverySource
    {
        struct ImpactRecord
        {
        ...
        [SerializeField] StonePreset _preset;
        [FormerlySerializedAs("profile")]
        [SerializeField] StoneAssemblyProfile _profile;
        ...
        readonly StoneAssembly _assembly = new StoneAssembly();
        ...
        public IReadOnlyList<StoneAssembly.Part> parts { get { return _assembly.parts; } }
        ...
        public void Init(Entity owner, StoneEffects effects)
        {
            ...
            {
                if (component is IStoneMotionSource source)
                {
                    _motion = source;
                    break;
                }
        ...
        public void Init(ResourceAttribute resource, uint visualSeed, StoneEffects effects)
        {
            ...
            if (_life == null)
            {
                _life = gameObject.AddComponent<StoneLife>();
            }
            _life.enabled = true;
            Transform cairnTop = null;
            if (_preset == StonePreset.Cairn)
            {
        ...
        public StoneImpact EstimateImpact(Vector3 queryWS, Vector3 incomingVelocityWS)
        {
            ...
            foreach (StoneAssembly.Part part in _assembly.parts)
            {
                ...
                if (StoneImpactLocator.TryClosestPoint(part.lease.data, part.transform.localToWorldMatrix, queryWS,
        ...
        void ShedPart()
        {
            ...
            else if (_preset == StonePreset.Monolith)
            {
                index = -1;
            }
        ...
        public void Collapse(StoneEffects owner = null)
        {
            if (owner != null)
            {
                _effects = owner;
            ...
            if (_effects != null)
            {
                _effects.CollapseOnce(this, StoneSeed.ForPart(_seed, 301));
            }
        ...
        public void AdvancePresentation(Vector3 targetDirection, float remaining, float deltaTime)
        {
            ...
            if (_preset != StonePreset.Boulder)
            {
        ...
        public bool BeginDelivery(int token, DeliveryStyle style, Transform projectile, Vector3 intendedEnd)
        {
            ...
            if (style != DeliveryStyle.Thrown && style != DeliveryStyle.Direct && style != DeliveryStyle.Rigid)
            {
                return false;
            }
