...
namespace HealerLike.Render.Stones
{
    ...
    {
        [SerializeField] Color groundColour=new Color32(46,125,79,255);
        public Color GroundColour
        {
            get=>groundColour;
            set { groundColour=value; if(material!=null) ApplyColour(); }
        }
        void ApplyColour() { Color c=groundColour*.65f; c.a=1; material.SetColor("_BaseColor",c.linear); }
        Transform disc;
        Mesh mesh;
        Material material;
        float radius;
        ...
        public void Configure(Bounds bounds)
        {
            ...
            if(disc==null)
            {
                disc=new GameObject("HLBareGround").transform; disc.SetParent(transform,false);
                const int n=32; var v=new Vector3[n+1]; var t=new int[n*3];
                for(int i=0;i<n;i++)
                {
                    float angle=i*Mathf.PI*2/n; v[i+1]=new Vector3(Mathf.Cos(angle),0,Mathf.Sin(angle));
                ...
                material=new Material(Shader.Find("Universal Render Pipeline/Lit")){name="HLBareGroundMaterial"};
