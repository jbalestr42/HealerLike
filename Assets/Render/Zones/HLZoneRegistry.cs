...
namespace HealerLike.Render.Zones
{
    ...
    public class HLZoneRegistry : MonoBehaviour, IHLZoneOwner
    {
        ...
        class GraphicsUpload : IHLZoneUpload
        {
            ...
            GraphicsBuffer _buffer = new GraphicsBuffer(GraphicsBuffer.Target.Structured, 64, HLZone.Stride);
        ...
        public int Add(HLZoneKind kind, Vector3 position, float radius, float strength)
        {
            ...
            if (!HLZonePacker.TryCreate(position, radius, kind, strength, 0, out HLZone zone) || zone.strength <= 0)
            {
                return 0;
            }
        ...
        public int AddLaunch(Vector3 source, Vector3 target)
        {
            ...
            {
                Entry entry = _entries[i];
                entry.zone.reserved = HLZonePacker.EncodeDirection(direction);
                _entries[i] = entry;
            }
        ...
        public void UpdateZone(int handle, HLZoneKind kind, Vector3 position, float radius, float strength)
        {
            ...
            Entry entry = _entries[i];
            if (!HLZonePacker.TryCreate(position, radius, kind, strength, entry.zone.age, out HLZone zone)
                || zone.strength <= 0)
            {
                _entries.RemoveAt(i);
                return;
        ...
        public void PublishFrame(float deltaTime)
        {
            ...
            {
                if (_source[i].kind != (int)HLZoneKind.Trample)
                {
                    feedback++;
                }
            }
            ...
            for (int i = 0; i < written; i++)
            {
                if (_source[i].kind == (int)HLZoneKind.Trample && footprints-- <= 0)
                {
                    continue;
                }
            ...
            _count = HLZonePacker.Pack(new ReadOnlySpan<HLZone>(_source, 0, selected), _packed, out _,
                                       out int overflow);
