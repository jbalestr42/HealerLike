...
namespace HealerLike.Render.Spells
{
    public class HLAttributeShieldView : MonoBehaviour, IVisualBehaviour
    {
        ...
        {
            if (attributes != manager || anchor != target)
            {
                Clear();
            }
            attributes = manager;
        ...
        public void Refresh()
        {
            ...
            {
                anchor = entity.targetPoint.transform;
            }
            float charges =
                attributes && attributes.Has(AttributeType.HitArmor) ? attributes.Get(AttributeType.HitArmor).Value : 0;
            if (!isActiveAndEnabled || !anchor || !(charges > 0) || float.IsInfinity(charges))
            {
                Clear();
                return;
            }
