...
namespace HealerLike.Render.Spells
{
    ...
    public sealed class HLSpellStyleTable : ScriptableObject
    {
        [Serializable] public struct HLEntry { public HLSpellSignature signature; public GameObject prefab; }
        public List<HLEntry> entries = new List<HLEntry>();
        public GameObject buff, shield, heal, impact, chain;
        public bool TryGet(HLSpellSignature signature, out GameObject prefab)
        {
            ...
            foreach (var entry in entries)
                ...
                { if (found) { prefab = null; return false; } found = true; prefab = entry.prefab; }
            return found && prefab;
        }
        public IReadOnlyList<int> FindCollisions()
        {
            var seen = new HashSet<HLSpellSignature>(); var indices = new List<int>();
            if (entries != null) for (int i = 0; i < entries.Count; i++) if (!seen.Add(entries[i].signature)) indices.Add(i);
            return indices;
        }
