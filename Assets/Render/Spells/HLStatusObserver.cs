    ...
    public sealed class HLStatusObserver : MonoBehaviour, IVisualBehaviour
    {
        ...
        public void Reconcile()
        {
            ...
            if(!ReferenceEquals(sink,_lastSink) || version != _sinkVersion)
            {
                ...
                _lastSink=sink; _sinkVersion=version; _published.Clear();
            }
            if(sink==null)return;
            if (_observed.Count == 0 && _published.Count == 0) return;
            _groups.Clear();
            foreach(var data in _observed)
            {
                ...
                if(_groups.TryGetValue(key,out var old)) _groups[key]=(old.stacks+stacks,Mathf.Min(old.elapsed,elapsed),Mathf.Max(old.duration,duration));
                else _groups[key]=(stacks,elapsed,duration);
            }
            foreach (var previous in _published)
                if (!_groups.ContainsKey(previous.Key)) sink.RemoveStatus(null,previous.Key.Item1,previous.Key.Item2);
            foreach(var group in _groups)
                if (!_published.TryGetValue(group.Key,out var old) || !old.Equals(group.Value))
                    sink.SetStatus(null,group.Key.Item1,group.Key.Item2,group.Value.stacks,group.Value.elapsed,group.Value.duration,HLClockKind.Simulation);
            var swap = _published; _published = _groups; _groups = swap;
        }
        void OnEnable() => Reconcile();
        void OnDisable()
        {
            foreach (var data in _observed) _lastSink?.RemoveStatus(null, data.target, data.buffHandlerFactory);
            _lastSink = null; _published.Clear();
        }
        ...
        public void Detach()
        {
            ...
            foreach(var data in _observed) _lastSink?.RemoveStatus(null,data.target,data.buffHandlerFactory);
            _observed.Clear();_groups.Clear();_published.Clear();_manager=null;_lastSink=null;
        }
    }
}
