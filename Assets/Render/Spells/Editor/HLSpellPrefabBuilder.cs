...
namespace HealerLike.Render.Spells
{
    public static class HLSpellPrefabBuilder
    {
        ...
        public static void Build()
        {
            ...
            SaveMesh(HLSpellPrimitives.Torus,"HLTorus");SaveMesh(HLSpellPrimitives.Cone,"HLCone");SaveMesh(HLSpellPrimitives.Star,"HLStar");SaveMesh(HLSpellPrimitives.Boulder,"HLBoulder");
            string[] names={"HLStatus_Buff","HLStatus_Shield","HLFx_HealSpheres","HLFx_Impact","HLFx_ChainBeam","HLFx_HostileLitter","HLFx_HealRing","HLFx_PoisonDrips"};
            HLSpellEffectKind[] kinds={HLSpellEffectKind.Buff,HLSpellEffectKind.Shield,HLSpellEffectKind.Heal,HLSpellEffectKind.Impact,HLSpellEffectKind.Chain,HLSpellEffectKind.Litter,HLSpellEffectKind.Area,HLSpellEffectKind.Drip};
            var prefabs=new GameObject[names.Length];
            for(int i=0;i<names.Length;i++)
            {
                var go=new GameObject(names[i]);var effect=go.AddComponent<HLSpellEffect>();effect.kind=kinds[i];if(effect.kind==HLSpellEffectKind.Litter)effect.lifetime=HLSpellVisualSink.PulseSeconds;effect.material=material;HLSpellPrimitives.Build(effect);
        ...
        static void AuthorSignatures(HLSpellStyleTable table, Material material)
        {
            ...
            foreach(var row in Inventory())
            {
                ...
                report.Append("| ").Append(row.Key).Append(" | ").Append(name).Append(" | ").Append(BeautyCode(HLSpellPrimitives.Kind(signature))).Append(" | ").Append(string.Join("<br>",row.Value.paths)).Append(" |\n");
            }
        ...
        static string BeautyCode(HLSpellEffectKind kind)
        {
            switch(kind)
            {
                case HLSpellEffectKind.Heal: return "Lime spheres / thin stalks / grow-pop";
                case HLSpellEffectKind.Impact: return "Coral flat star / four gravity shards";
                case HLSpellEffectKind.Shield: return "Six overlapping leaf plates / close-open";
                case HLSpellEffectKind.Drip: return "Coral drops / observed tick period";
                case HLSpellEffectKind.Mana: return "Hollow gold resource beads";
                default: return "Three tilted tori / orbit / soft pulse";
            }
        }
        static void PersistBeautyMeshes(GameObject go)
        {
            foreach(var filter in go.GetComponentsInChildren<MeshFilter>())
            {
                if(filter.sharedMesh==HLSpellPrimitives.Star) filter.sharedMesh=AssetDatabase.LoadAssetAtPath<Mesh>(Root+"Data/HLStar.asset");
                else if(filter.sharedMesh==HLSpellPrimitives.Boulder) filter.sharedMesh=AssetDatabase.LoadAssetAtPath<Mesh>(Root+"Data/HLBoulder.asset");
            }
        }
