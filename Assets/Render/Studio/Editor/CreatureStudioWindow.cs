...
namespace HealerLike.Render.Studio.Editor
{
    public sealed partial class CreatureStudioWindow : EditorWindow
    {
        ...
        internal static void NotifyRecipeChanged(CreatureRecipe recipe)
        {
            ...
            RenderGrammarLibraryWindow.NotifyAssetChanged(recipe);
        }
        ...
        private void OnUndo() { serialized?.Update(); if (grammarMode) RegenerateGrammar(); else RenderGrammarLibraryWindow.NotifyAssetChanged(selected); RefreshPreview(); Repaint(); }
        ...
        private LookSide RecipeSurface(CreatureRecipe recipe)
        {
            ...
            return !string.IsNullOrEmpty(guid) && EditorPrefs.HasKey(key) ? (LookSide)EditorPrefs.GetInt(key) : SpellStudioPreview.InferReferenceSide(recipe);
        }
        ...
        private void ApplyEdits() { serialized.ApplyModifiedProperties(); RefreshPreview(); RenderGrammarLibraryWindow.NotifyAssetChanged(selected); }
        ...
        private void FinishMutation() { EditorUtility.SetDirty(selected); serialized.Update(); RefreshPreview(); Repaint(); RenderGrammarLibraryWindow.NotifyAssetChanged(selected); }
