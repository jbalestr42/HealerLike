...
namespace HealerLike.Render.Stones
{
    public class HLStoneMeshTests
    {
        ...
        public void CountsAndFlatNormals(int n, int count)
        {
            ...
            HLStoneMeshData data = HLStoneMesh.Generate(0, settings);
        ...
        public void DeterministicAndIndependentOfGlobalRandom(uint seed)
        {
            ...
            try
            {
                HLStoneMeshData a = HLStoneMesh.Generate(seed, HLStonePresets.Boulder);
                Assert.AreEqual(state, Random.state);

                Random.InitState(128);
                HLStoneMesh.Generate(seed + 1, HLStonePresets.Monolith);
                HLStoneMeshData b = HLStoneMesh.Generate(seed, HLStonePresets.Boulder);
                ...
                HLStoneMeshData next = HLStoneMesh.Generate(seed + 1, HLStonePresets.Boulder);
        ...
        {
            HLStoneSettings[] shapes = { HLStonePresets.Boulder, HLStonePresets.Cairn, HLStonePresets.Monolith };
            float[] roughnessValues = { 0f, 0.18f };
            for (int n = 0; n <= 2; n++)
            {
                foreach (HLStoneSettings shape in shapes)
                {
                    foreach (float rough in roughnessValues)
                    {
                        for (int axes = 0; axes < 8; axes++)
                        {
                            ...
                            {
                                HLStoneSettings settings = shape;
                                settings.subdivisions = n;
                                ...
                                CheckFaces(HLStoneMesh.Generate(seed, settings), settings, seed, n);
                            }
                        }
                    }
                }
        ...
        static void CheckFaces(HLStoneMeshData data, HLStoneSettings settings, uint seed, int n)
        {
        ...
        public void CanonicalGeometryHashIsPinned()
        {
            ...
            HLStoneMeshData data = HLStoneMesh.Generate(827361, HLStonePresets.Boulder);
            ...
            {
                hash = HLStoneSeed.ForPart(hash, (uint)index);
            }
        ...
        static uint HashVector(uint hash, Vector3 value)
        {
            ...
            {
                hash = HLStoneSeed.ForPart(hash, unchecked((uint)System.BitConverter.SingleToInt32Bits(component)));
            }
            return hash;
        }
