...
public class GrassBenchmarkTests
{
    GameObject _go;
    GrassBenchmark _benchmark;
    ...
    {
        for (int i = 0; i < GrassBenchmark.WarmupFrames; i++)
        {
            Assert.IsFalse(_benchmark.RecordFrame(999));
        }
        for (int i = 0; i < GrassBenchmark.SampleFrames - 1; i++)
        {
    ...
    {
        for (int i = 0; i < GrassBenchmark.WarmupFrames + 5; i++)
        {
