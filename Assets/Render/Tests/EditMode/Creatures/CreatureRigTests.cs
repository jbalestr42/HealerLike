...
namespace HealerLike.Render.Creatures
{
    public class CreatureRigTests
    {
        ...
        public void ShortDirectDeliveryDoesNotDrawUnusedBoardLengthAsCoils()
        {
            ...
            try
            {
                using (CreatureRig body = CreateRig(data, host.transform, material))
                {
                    body.Tick(0f, 0f, new FootFrame(Vector3.zero, Vector3.up, 1f));
                    Vector3 target = new Vector3(2f, 1.3f, 0f);

                    Assert.IsTrue(body.BeginDelivery(500, DeliveryStyle.Direct, null, target));
