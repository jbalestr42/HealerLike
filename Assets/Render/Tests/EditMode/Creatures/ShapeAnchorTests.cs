...
public class ShapeAnchorTests
{
    ...
    public void Anchor_CurvedGrowthAndSeededMinerals_MatchesActualMeshEndFaceCentres()
    {
        ...
        foreach (
            ...
            {
                ShapeProfile.Leaf(0.85f, 1.7f),
                ShapeProfile.Segment(0.65f, 0.4f, -0.7f),
                block,
                ShapeProfile.Shard(),
                ShapeProfile.Bulb(2.2f, -0.5f),
                ShapeProfile.Ring(),
                ShapeProfile.Ring(0.3f, true),
            }
        )
        {
    ...
    public void Anchor_LegacyUsesNormalizedAxisAndInvalidInputsAreRejected()
    {
        ...
        Assert.IsFalse(ShapeAnchors.TryGet(default, (ShapeAnchor)99, 0, out _));
        ShapeProfile invalid = ShapeProfile.Leaf();
        invalid.bend = float.NaN;
        Assert.IsFalse(ShapeAnchors.TryGet(invalid, ShapeAnchor.Bottom, 0, out _));
        Assert.IsFalse(ShapeAnchors.TryGet(invalid, ShapeAnchor.Center, 0, out _));
    }
}
}
