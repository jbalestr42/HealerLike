...
public class FragmentPlacementTests
{
    static LookPart[] Chain()
    {
        return new[]
        {
            new LookPart
            {
                ...
                position = new Vector3(0.2f, 0.1f, 0f),
                size = new Vector3(0.3f, 0.8f, 0.3f),
            ...
            new LookPart
            {
                ...
                size = new Vector3(0.55f, 1.3f, 0.24f),
                euler = new Vector3(0f, 40f, -15f),
            },
            new LookPart
            {
                id = "Accent",
    ...
    public void Resolve_ProfileEdits_PreserveBasalContactAndMoveTheDependentAccent(
        float bend,
        float fullness,
        float taper
    )
    {
        ...
        Assert.IsTrue(
            FragmentPlacement.TryResolve(fragment, CountBand.One, 17, 0, out LookPart[] after, out string error),
            error
        );
        ...
        Assert.Greater(
            Vector3.Distance(Anchor(before[2], ShapeAnchor.Bottom), Anchor(after[2], ShapeAnchor.Bottom)),
            0.001f
        );
    }
    ...
    public void Resolve_DefaultMetadata_KeepsLegacyPartCentresUnchanged()
    {
        ...
        Assert.IsTrue(
            FragmentPlacement.TryResolve(fragment, CountBand.One, 17, 4, out LookPart[] placed, out string error),
            error
        );
        for (int i = 0; i < fragment.Length; i++)
        {
    ...
    public void Resolve_InvalidReference_RejectsWithoutPartialPlacement(string failure)
    {
        ...
        if (failure == "missing")
        {
            fragment[1].attachTo = "Absent";
        }

        if (failure == "forward")
        {
            fragment[0].attachTo = "Blade";
        }
        ...
        if (failure == "inactive")
        {
            fragment[0].minCount = CountBand.Few;
        }

        Assert.IsFalse(
            FragmentPlacement.TryResolve(fragment, CountBand.One, 17, 0, out LookPart[] placed, out string error)
        );
    ...
    public void Resolve_ConditionalChain_ActivatesTogetherAtTheCountBand()
    {
        ...
        Assert.IsTrue(
            FragmentPlacement.TryResolve(fragment, CountBand.One, 17, 0, out LookPart[] one, out string oneError),
            oneError
        );
        Assert.IsTrue(
            FragmentPlacement.TryResolve(fragment, CountBand.Few, 17, 0, out LookPart[] few, out string fewError),
            fewError
        );
    ...
    public void Resolve_InvalidAnchor_RejectsBeforeQueryingTheGenerator(bool ownPivot)
    {
        ...
        if (ownPivot)
        {
            fragment[1].pivot = (ShapeAnchor)999;
        }
        else
        {
            fragment[1].attachAt = (ShapeAnchor)999;
        }
