...
public class CreatureAppearanceTests
{
    ...
    {
        Assert.That(
            CreatureAppearance.RootDelay(0, 6, 3, 4),
    ...
    public void Rig_AppearanceIsPoseRelativeWithoutCompoundingOrReplayingOnRecompose(LookSide side)
    {
        ...
        try
        {
            ...
            {
                authored[i] = rig.partTransforms[i].localScale;
            }
            ...
            {
                growing[i] = rig.partTransforms[i].localScale;
            }
            ...
            for (int i = 0; i < growing.Length; i++)
            {
                ...
                if (growing[i].sqrMagnitude < authored[i].sqrMagnitude * 0.5f)
                {
                    partial = true;
                }
            }
