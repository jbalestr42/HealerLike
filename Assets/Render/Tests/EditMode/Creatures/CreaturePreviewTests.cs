...
public class CreaturePreviewTests
{
    ...
    public void Preview_UsesLiveSideRecipeAndMaterialsWithoutGameplayAndReleasesOwnedRecipe(Entity.EntityType side)
    {
        ...
        Assert.That(
            _preview.Init(_looks, data, side, RenderTestAssets.LoadMeshes(), _parent.transform, 1f),
            Is.True
        );
    ...
    public void Preview_AuthoredOverrideUsesItsRecipeAndDoesNotDestroyIt()
    {
        ...
        try
        {
            ...
            Assert.That(
                _preview.Init(looks, data, Entity.EntityType.Computer, null, _parent.transform, 1f),
                Is.True
            );
            Assert.That(_preview.rig.recipe, Is.SameAs(recipe));
            Assert.That(
                _preview.rig.partTransforms[0].GetComponent<Renderer>().sharedMaterial,
                Is.SameAs(material)
            );
            _preview.Dispose();
    ...
    public void Preview_InvalidInputLeavesParentEmptyAndCanBeDisposed()
    {
        ...
        Assert.That(
            _preview.Init(
                _looks,
                RenderTestAssets.LoadEntity("NormalEntity"),
