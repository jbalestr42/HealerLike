...
namespace HealerLike.Render.Creatures
{
    public class OffscreenPlayerTests
    {
        ...
        public void SingleHealthOutcome_DrawsLinkFromScreenAndKeepsItThereWhenCameraMoves()
        {
            ...
            link.Advance(0f);
            Vector3 start = (Vector3)typeof(SpellEffect).GetField("_linkStart",
                BindingFlags.Instance | BindingFlags.NonPublic).GetValue(link);
        ...
        SpellEffect Link()
        {
            ...
            {
                if (effect.recipe.socket == EffectSocket.Link) return effect;
            }
            return null;
        }
        ...
        public void StatusCast_PublishesOnApplicationAndStackIncreaseWithoutRepeatingOnClockUpdates()
        {
            ...
            for (int i = 1; i < 10; i++) _sink.SetStatus(_source, _target, factory, 1, i * 0.1f, 8f);
            Assert.AreEqual(1, _sink.impactCount);
        ...
        public void PlayerProjectile_MapsVisualEntryAndLandsWithoutChangingLogicalTrajectory(DeliveryStyle style)
        {
            ...
            Assert.That(_camera.WorldToViewportPoint(shot.visualPosition).y,
