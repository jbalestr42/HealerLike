...
namespace HealerLike.Render.Zones
{
    ...
    public class HLZoneRegistryTests
    {
        ...
        int Add(float x)
        {
            return _registry.Add(HLZoneKind.Heal, new Vector3(x, 0f, 0f), 1f, 1f);
        }
        ...
        public void ActsAsTheRegistryZoneOwner()
        {
            ...
            int handle = owner.AddPulse(HLZoneKind.Hostile, Vector3.zero, 2f, 1f, 0.8f);
        ...
        public void UpdatesPreserveOrderAndPublishCanonicalSnapshotEveryFrame()
        {
            ...
            Add(2f);
            Add(3f);
            _registry.UpdateZone(first, HLZoneKind.Hostile, Vector3.right * 9f, 2f, 5f);
            _upload.calls.Clear();
        ...
        public void PersistentFootprintsCannotStarveHealAndExpiredFeedbackReturnsCapacity()
        {
            for (int i = 0; i < 80; i++)
            {
                _registry.Add(HLZoneKind.Trample, Vector3.right * i, 1f, 1f);
            }

            int heal = _registry.AddPulse(HLZoneKind.Heal, Vector3.right * 100f, 2f, 1f, 0.45f);
            _registry.AddPulse(HLZoneKind.Hostile, Vector3.right * 101f, 2f, 1f, 0.8f);
            LogAssert.Expect(LogType.Warning, overflowWarning);
        ...
        public void RemovedSlotsAreReusedWithoutReusingHandlesOrReordering()
        {
            ...
            _registry.UpdateZone(old, HLZoneKind.Hostile, Vector3.zero, 9f, 1f);
            _registry.PublishFrame(0f);
        ...
        {
            int pulse = _registry.AddPulse(HLZoneKind.Hostile, Vector3.one, 2f, 0.8f, 0.8f);
        ...
        {
            Assert.AreEqual(0, _registry.Add(HLZoneKind.None, Vector3.zero, 1f, 1f));
            Assert.AreEqual(0, _registry.Add(HLZoneKind.Heal, Vector3.zero, 0f, 1f));
            Assert.AreEqual(0, _registry.Add(HLZoneKind.Heal, Vector3.zero, 1f, 0f));
            Assert.AreEqual(0, _registry.AddPulse(HLZoneKind.Heal, Vector3.zero, 1f, 1f, float.NaN));
            Assert.AreEqual(0, _registry.AddPulse(HLZoneKind.Heal, Vector3.zero, 1f, 1f, 0f));
