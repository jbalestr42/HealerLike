...
public class ZoneUpdateTests
{
    ...
    [Test]
    public void UpdateZone_Launch_KeepsHeadingAgeOrderAndRemainingFade()
    {
        _registry.Add(ZoneKind.Heal, Vector3.left, 1f, 1f);
        int launch = _registry.AddLaunch(Vector3.zero, Vector3.forward * 5f);
        _registry.Add(ZoneKind.Heal, Vector3.right, 1f, 1f);
        _registry.PublishFrame(0.1f);
        uint heading = _registry.snapshot[1].reserved;
        Assert.AreNotEqual(0u, heading);

        _registry.UpdateZone(launch, ZoneKind.Launch, Vector3.back, 3f, 0.8f);
        ...
        Assert.AreEqual(heading, _registry.snapshot[1].reserved);
    ...
    [Test]
    public void UpdateZone_KindChanges_ClearsThePreviousLaunchHeading()
    {
        int handle = _registry.AddLaunch(Vector3.zero, Vector3.forward * 5f);
        _registry.PublishFrame(0.1f);
        ...
        _registry.UpdateZone(handle, ZoneKind.Launch, Vector3.one, 2f, 1f);
        _registry.PublishFrame(0f);
        Assert.AreEqual((int)ZoneKind.Launch, _registry.snapshot[0].kind);
        Assert.AreEqual(0u, _registry.snapshot[0].reserved);
    }
}
