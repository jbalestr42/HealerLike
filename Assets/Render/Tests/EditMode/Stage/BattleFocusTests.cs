...
public class BattleFocusTests
{
    ...
    public void FitsEveryBodyCornerWithHudMargin(float aspect,float width,float height,float depth)
    {
        ...
        var pose=BattleFocusBounds.Fit(b,52,40,aspect);
        for(int x=-1;x<=1;x+=2) for(int y=-1;y<=1;y+=2) for(int z=-1;z<=1;z+=2)
        {
            var world=b.center+Vector3.Scale(b.extents,new Vector3(x,y,z));
            var point=EnvironmentForeground.ToViewport(world,pose.position,pose.rotation,40,aspect);
    ...
    [Test] public void SpreadWidensViewAndTranslationOnlyMovesPose()
    {
        ...
        var a=BattleFocusBounds.Fit(small,52,40,.5625f); var b=BattleFocusBounds.Fit(wide,52,40,.5625f);
        Assert.That(b.position.magnitude,Is.GreaterThan(a.position.magnitude*2));
        var offset=new Vector3(13,4,-7); small.center+=offset;
        Assert.That(Vector3.Distance(BattleFocusBounds.Fit(small,52,40,.5625f).position,a.position+offset),Is.LessThan(.0001f));
    }
}
