...
public class HLGrassWindTests
{
    static HLGrassWind CreateWind(Vector2 direction, float amplitude)
    {
        HLGrassWind wind = new HLGrassWind();
    ...
    {
        HLGrassWind wind = CreateWind(new Vector2(3f, 4f), amplitude: 0.05f);
    ...
    {
        HLGrassWind wind = CreateWind(Vector2.zero, amplitude: 0.05f);
    ...
    {
        HLGrassWind wind = CreateWind(Vector2.right, amplitude: 3f);

        Assert.AreEqual(HLGrassWind.MaxAmplitude, wind.current.w);
    }

    [Test]
    public void TriggerGust_TowardTarget_PointsAtTargetAndDoublesAmplitude()
    {
        HLGrassWind wind = CreateWind(Vector2.right, amplitude: 0.065f);
