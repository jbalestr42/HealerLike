...
namespace HealerLike.Render.Spells
{
    public class HLDeliveryStylesTests
    {
        ...
        public void FactsSelectDelivery(bool curve,bool laser,bool swarm,bool bounce,bool chain,HLDeliveryStyle expected)
            => Assert.AreEqual(expected,HLDeliveryStyles.Classify(curve,laser,swarm,bounce,chain));
        [Test] public void NamesResolveOriginalStageAndCloneWithoutGameplay()
        {
            var table=ScriptableObject.CreateInstance<HLDeliveryStyles>();
            var go=new GameObject("HLCurveBullet(Clone)");
            try
            {
                table.entries.Add(new HLDeliveryStyles.HLEntry{prefabName="CurveBullet",style=HLDeliveryStyle.Arc});
        ...
        [Test] public void ShippedTableHasNineUniqueVariants()
        {
            var table=AssetDatabase.LoadAssetAtPath<HLDeliveryStyles>("Assets/Render/Spells/Data/HLDeliveryStyles.asset");
