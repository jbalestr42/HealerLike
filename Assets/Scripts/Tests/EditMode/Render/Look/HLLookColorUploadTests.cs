...
namespace HealerLike.Render.Look
{
    public class HLLookColorUploadTests
    {
        [Test] public void WorkingSpacePropertyVectorsReachPrimitiveShaderUnchanged()
        {
            ...
            var mesh=new Mesh { vertices=new[]{new Vector3(-0.8f,-0.8f,0),new Vector3(-0.8f,0.8f,0),new Vector3(0.8f,0.8f,0),new Vector3(0.8f,-0.8f,0)}, triangles=new[]{0,1,2,0,2,3}, normals=new[]{Vector3.back,Vector3.back,Vector3.back,Vector3.back} };
            ...
            try {
                ...
                var artist=new Color(0.4f,0.6f,0.8f,1);var linear=artist.linear;
                Color Read(int mode) {
                ...
                Assert.That(((Vector4)a-(Vector4)c).magnitude,Is.LessThan(0.004f),"Artist SetColor and working SetVector must agree");
                Assert.That(((Vector4)b-(Vector4)c).magnitude,Is.GreaterThan(0.2f),"Double conversion must be detected by GPU readback");
                Assert.That(((Vector4)c-(Vector4)linear).magnitude,Is.LessThan(0.004f),"Explicit working vector should reach GPU unchanged");
