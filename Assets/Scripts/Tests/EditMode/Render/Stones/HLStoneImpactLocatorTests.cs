...
namespace HealerLike.Render.Stones
{
    public class HLStoneImpactLocatorTests
    {
        ...
        public void ClosestFacePointUnderRotationAndNonuniformScale()
        {
            ...
            HLStoneMeshData data = new HLStoneMeshData(vertices, normals, new int[] { 0, 1, 2 }, default);
            ...
            bool found = HLStoneImpactLocator.TryClosestPoint(data, matrix, query,
            ...
            Assert.IsTrue(HLStoneImpactLocator.TryClosestPoint(data, Matrix4x4.identity, query, out point, out normal));
            Assert.AreEqual(Vector3.zero, point);

            Assert.IsFalse(HLStoneImpactLocator.TryClosestPoint(default, matrix, Vector3.zero, out point, out normal));
        }
    }
}
